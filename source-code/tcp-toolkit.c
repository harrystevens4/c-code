...
int make_socket(const char * port, struct addrinfo **res){
	int socketfd;
	...
	int result = getaddrinfo(NULL, port, &hints, res); //fill in addrinfo struct 'res'
	if (result != 0){
...
int make_server_socket(const char * port, int backlog){
	struct addrinfo *res;
	int socket = make_socket(port, &res);
	if (socket < 0){
		return -1;
	}
